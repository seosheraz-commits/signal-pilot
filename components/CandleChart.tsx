...
async function fetchKlinesAPI(ex: Exchange, mk: Market, sym: string, tf: string, limit = 500): Promise<Bar[]> {
  ...
  if (j?.error || !Array.isArray(j?.candles)) throw new Error(j?.error || 'klines failed');
  // Binance/MEXC candle shape: [openTime, open, high, low, close, volume, ...]
  return j.candles.map((k: any[]) => ({
...
async function fetchTickerAPI(ex: Exchange, mk: Market, sym: string): Promise<number | null> {
...
export default function CandleChart({
  ...
  function computeAndDecorate(hotPrice?: number) {
    ...
    if (config.overlays.signalLevels && (label === 'Long' || label === 'Short')) {
      ...
      entryLineRef.current = seriesRef.current?.createPriceLine({
        price: entryF, color: '#facc15', lineWidth: 2, title: `Entry ${entryF.toFixed(6)}`
      });
      slLineRef.current = seriesRef.current?.createPriceLine({
        price: slF, color: '#ef4444', lineWidth: 2, title: `SL ${slF.toFixed(6)}`
      });
      tpLineRef.current = seriesRef.current?.createPriceLine({
        price: tpF, color: '#22c55e', lineWidth: 2, title: `TP ${tpF.toFixed(6)}`
      });
    }
    ...
    supportLineRef.current = seriesRef.current?.createPriceLine({
      price: recentLow, color: '#06b6d4', lineWidth: 1, title: `Support ${recentLow.toFixed(6)}`
    });
    resistanceLineRef.current = seriesRef.current?.createPriceLine({
      price: recentHigh, color: '#a855f7', lineWidth: 1, title: `Resistance ${recentHigh.toFixed(6)}`
    });
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // history (via our API so spot/futures + both exchanges work)
      try {
        const data = await fetchKlinesAPI(exchange as Exchange, market as Market, symbol, interval);
        if (disposed) return;
      ...
      // seed live
      const seed = await fetchTickerAPI(exchange as Exchange, market as Market, symbol);
      if (seed) {
        setLivePriceLine(seed);
        computeAndDecorate(seed);
      }

      // Polling loop (works for all combos reliably; avoids CORS/WS differences)
      pollKlinesRef.current = setInterval(async () => {
        try {
          const arr = await fetchKlinesAPI(exchange as Exchange, market as Market, symbol, interval);
          if (!arr.length) return;
          const latest = arr[arr.length - 1];
          const prevLast = barsRef.current[barsRef.current.length - 1]?.time ?? 0;
          barsRef.current = arr;
          series.setData(arr);
          if (latest.time !== prevLast) computeAndDecorate();
        } catch {}
      }, 2000);

      pollTickerRef.current = setInterval(async () => {
        try {
          const p = await fetchTickerAPI(exchange as Exchange, market as Market, symbol);
          if (p) {
            setLivePriceLine(p);
