...
export async function GET(req: Request) {
  try {
    ...
    // Discover universe
    const symURL = new URL(req.url);
    symURL.pathname = '/api/symbols';
    symURL.searchParams.set('exchange', exchange === 'both' ? 'all' : exchange);
    symURL.searchParams.set('market',   market   === 'both' ? 'all' : market);
    const { symbols } = await (await fetch(symURL, { cache:'no-store' })).json();
    const universe = (symbols || []).filter((s:any)=>s.quote==='USDT').slice(0, cap);
    ...
    for (let i=0;i<universe.length;i+=batch) {
      ...
      await Promise.all(chunk.map(async (s:any) => {
        ...
        k.searchParams.set('limit',    '200');
        try {
          const { candles } = await (await fetch(k.toString(), { cache:'no-store' })).json();
    ...
    return NextResponse.json({ picks: picks.slice(0,3), scanned: universe.length, mode, interval }, { headers:{ 'cache-control':'no-store' } });
  } catch (e: any) {
