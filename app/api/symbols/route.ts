...
async function get<T>(url: string) {
  const r = await fetch(url, {
    ...
    // keep requests simple; some MEXC endpoints are touchy with headers
    next: { revalidate: 0 },
  });
...
/* ---------------- MEXC (aggressive + resilient) ----------------
   We union multiple endpoints and de-dup. This avoids the "only 5 coins"
   issue caused by a truncated region response from a single endpoint.
-----------------------------------------------------------------*/

// MEXC SPOT sources (all are on api.mexc.com to avoid cross-domain quirks):
...
async function mexcFutures(): Promise<SymbolInfo[]> {
  ...
  try {
    ...
    for (const s of d?.data ?? []) {
      ...
      const sym = raw.replace('_', ''); // BTCUSDT
  ...
  // 2) ticker (huge list; very reliable)
  try {
